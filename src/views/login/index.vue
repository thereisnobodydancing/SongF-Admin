<template>
  <div class="bg-white dark:bg-gray-900">
    <div class="flex justify-center h-screen">
      <div 
        class="hidden bg-cover lg:block lg:w-2/3"
        style="background-image: url(https://images.unsplash.com/photo-1616763355603-9755a640a287?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80)"
      >
        <div class="flex items-center h-full px-20 bg-gray-900 bg-opacity-40">
          <div>
            <h2 class="text-4xl font-bold text-white">光大后台管理系统</h2>
            <p class="max-w-xl mt-3 text-gray-500">Everbright Health Background Management System - a background management system integrating mall management, content management, user management, data center and system management</p>
          </div>
        </div>
      </div>

      <div class="flex items-center w-full max-w-md px-6 mx-auto lg:w-2/6">
        <div class="flex-1">
          <div class="text-center">
            <h2 class="text-4xl font-bold text-center text-gray-600 dark:text-white">欢迎登录</h2>
            <p class="mt-3 text-gray-500 dark:text-gray-500">请输入您的账号密码</p>
          </div>

          <div class="mt-8">
            <form>
              <div>
                <label for="email" class="block mb-2 text-sm text-gray-600 dark:text-gray-200">账号</label>
                <input 
                  v-model="form.phone"
                  type="text" name="email" id="email" placeholder="请输入您的账号"
                  class="block w-full px-4 py-2 mt-2 text-gray-700 placeholder-gray-400 bg-white border border-gray-200 rounded-md dark:placeholder-gray-600 dark:bg-gray-900 dark:text-gray-500 dark:border-gray-700 focus:border-blue-400 dark:focus:border-blue-400 focus:ring-blue-400 focus:outline-none focus:ring focus:ring-opacity-40" />
              </div>

              <div class="mt-6">
                <div class="flex justify-between mb-2">
                  <label for="password" class="text-sm text-gray-600 dark:text-gray-200">密码</label>
                  <span 
                    class="text-sm text-gray-400 cursor-pointer focus:text-blue-500 hover:text-blue-500 hover:underline"
                    @click="forget"
                  >
                    忘记密码?
                  </span>
                </div>

                <input 
                  v-model="form.password"
                  type="password" name="password" id="password" placeholder="请输入您的密码"
                  class="block w-full px-4 py-2 mt-2 text-gray-700 placeholder-gray-400 bg-white border border-gray-200 rounded-md dark:placeholder-gray-600 dark:bg-gray-900 dark:text-gray-500 dark:border-gray-700 focus:border-blue-400 dark:focus:border-blue-400 focus:ring-blue-400 focus:outline-none focus:ring focus:ring-opacity-40" />
              </div>

              <div class="mt-6">
                <button
                  :disabled="btnDisabled"
                  class="w-full px-4 py-2 tracking-wide text-white transition-colors duration-200 transform bg-blue-500 rounded-md hover:bg-blue-400 focus:outline-none focus:bg-blue-400 focus:ring focus:ring-blue-300 focus:ring-opacity-50"
                  @click="login"
                >
                  登录
                </button>
              </div>
            </form>

            <p class="mt-6 text-sm text-center text-gray-400">
              还没有帐户？<span class="text-blue-500 cursor-pointer focus:outline-none focus:underline hover:underline" @click="regisiter">注册。</span>
            </p>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup>
import api from '/src/api/index.js'
import Encrypt from '/src/until/crypto.js'

const router = useRouter()
const form = reactive({
  loginType: true,
  phone: localStorage.getItem('phone') ? localStorage.getItem('phone') : '',
  password: '',
  captcha: ''
})

const regisiter = function() {
  ElMessage({ message: '你没有注册资格', type: 'warning'})
}
const forget = function() {
  ElMessage({ message: '真厉害，密码都能忘', type: 'warning'})
}

const btnDisabled = ref(false)

const login = function () {
  if (!form.phone || !form.password) { ElMessage({ message: '请填写完整信息', type: 'error'}); return }
  btnDisabled.value = true
  let postData = {}
  postData = Object.assign(postData, form)
  postData.password = Encrypt.Encrypt(postData.password)
  api.post('/open/login', postData).then((res) => {
    if (res.data.code === 20000) {
      sessionStorage.setItem('token', res.data.data.token)
      localStorage.setItem('phone', res.data.data.user.mobile)
      sessionStorage.setItem('user', JSON.stringify(res.data.data.user))
      ElMessage({ message: `欢迎你，${res.data.data.user.name}`, type: 'success'})
      router.push('/')
    }
    setTimeout(() => btnDisabled.value = false, 500)
  })
}
</script>